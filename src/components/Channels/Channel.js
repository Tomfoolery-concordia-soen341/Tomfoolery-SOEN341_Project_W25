...
import {doc, updateDoc, arrayUnion, getDoc,where ,collection, orderBy, getDocs, onSnapshot, addDoc, serverTimestamp, arrayRemove, query} from "firebase/firestore";
                        ...
                        <ul className="scrollable-content members-list">
                            {membersWithStatus.map((member, index) => (
                                <div key={index} className="user-card">
                                    <span
                                        className={`status-indicator ${isOnline(member.status) ? 'status-online' : 'status-offline'}`}>
                                    </span>
                                    ...
                                    <div className="user-status">
                                        {isOnline(member.status) ? "Online" : formatLastSeen(member.lastSeen, member.status)}
                                    </div>
                ...
                <div className="chat-area">
                                    ...
                                    {msg.quotedMessage && (
                                        <div className="quoted-message">
                                            <p>
                                                ...
                                                <div>{msg.quotedMessage.sender}: {msg.quotedMessage.text}</div>
                                            </p>
                                        </div>
                                    )}
                                </li>
                    ...
                    {quotedMessage && (
                        <div className="quoted-message-preview">
                            <p>
                                <strong>Replying to</strong>
                                <div>{quotedMessage.sender}: {quotedMessage.text}</div>
                            </p>
                            <button className="remove-quote-btn" onClick={() => setQuotedMessage(null)}>Remove Quote</button>
                        </div>
                    )}
            ...
            <div className="left-column">
                <div className="channel-info">
                    ...
                    {!admin && !channel.isDefault && (
                        <button className="leave-channel-btn" onClick={leaveChannel}>Leave Channel</button>
                    )}
                    ...
                    <button className="back-btn" onClick={BackToDashboard}>Go back to Dashboard</button>
                </div>
            </div>
        </div>
    );
